...
exports.login = async (req, res) => {
  ...
  try {
    ...
    res.json({ token, user: { id: user.id, username: user.username, email: user.email, phone: user.phone, role: user.role, credits: user.credits } });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
exports.updateProfile = async (req, res) => {
  ...
  try {
    const result = await db.query(
      'UPDATE users SET username = $1, phone = $2 WHERE id = $3 RETURNING id, username, email, phone, role, credits',
      [username, phone, id]
    );
